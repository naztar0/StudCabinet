...
def histogram(line_count, percent, name_text, full_name=None):
    ...
    for i in range(1, line_count + 1):  # while cols are
        ...
        draw.line(((x, height), (x, height - line_height)), fill=color, width=line_width)  # draw column
        #          ^ 1-st point ^ 2-nd point, subtracted from height because OX-mirroring is necessary

        text_vertical_shift = height - line_height - line_width  # space between column and top integer
        ...
        im.paste(ImageOps.colorize(vert_text, (0, 0, 0), (color[0] + 100, color[1] + 100, color[2] + 100)),
                 #                                       ^ a little lighter color than on column
